...
// Si es un 1 es obligatorio, si es un 0 no lo es
     ...
     * @since 2020-10-22
     * @version Se ha añadido dos condiciones para que en caso de ser opciona y tener un mínimo o máximo establecedido no salte un error por ser inferior al mínimo o superior al máximo
     */
       ...
       $mensajeError = null;
        if ($obligatorio == 1 && $integer != '0') {
            ...
            if ($integer != '0' && !empty($integer) && !filter_var($integer, FILTER_VALIDATE_INT)) {  //HE VARIADO ESTO (COMPRUEBA SI ES 0, EVITA MOSTRAR MENSAJE SI ES OPCIONAL Y COMPRUEBA QUE SEA UN ENTERO)
                ...
                if ($integer > $max && !empty($integer)) {//Compruebas que integer no sea mayor que max porque esta vacio
                    $mensajeError = $mensajeError . "El número no puede ser mayor que " . $max . ".";
                }
                if ($integer < $min && !empty($integer)) {//Compruebas que integer no sea menor que min porque esta vacio
...
// Devuelve True si es una fecha valida y un false si no la es
     ...
     * @version 1.4 Corregido la existencia de dos variables que almacenan el mensaje de error, se ha renombrado a todas las variables del estilo mensajeError al ser el mas descriptivo
     */
